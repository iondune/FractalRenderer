...
class CBufferObject
{
    ...
    void allocate(unsigned int const size, void * data = 0)
    {
        ...
        glBindBuffer(IndexBuffer ? GL_ELEMENT_ARRAY_BUFFER : GL_ARRAY_BUFFER, this->Handle);
        glBufferData(IndexBuffer ? GL_ELEMENT_ARRAY_BUFFER : GL_ARRAY_BUFFER, size * sizeof(T), data, this->Usage);
        AllocatedSize = size;
    }
...
public:
    ...
    virtual void syncData()
    {
        ...
        else
        {
            ...
            {
                glBindBuffer(IndexBuffer ? GL_ELEMENT_ARRAY_BUFFER : GL_ARRAY_BUFFER, this->Handle);
                glBufferSubData(IndexBuffer ? GL_ELEMENT_ARRAY_BUFFER : GL_ARRAY_BUFFER, 0, this->Elements.size() * sizeof(T), & Elements[0]);
            }
            else
            {
                reallocate(this->Elements.size(), & Elements[0]);
            }
        }
